...
* **王沛东 (Peidong Wang)**
  ...
  * **工程职责**：后端基础设施与 API 逻辑（基于 FastAPI 的异步 RESTful API 研发，封装底层模型推理，提供稳定的 RAG 接口）。
* **罗天 (Tian Luo)**
...
* **袁晨宇 (Chenyu Yuan)**
  ...
  * **工程职责**：Interactive UI 开发（构建 Streamlit 多轮对话检索界面，图表和推荐卡片展示，开发交互式人工标注 UI）。

---

### **6周日程表**

#### **第一阶段：核心 Pipeline 基线与验证（第 1-2 周）**
...
* **交付产物 (Explicit Outputs)**：
  ...
  3. **前端展示界面**：完成 Streamlit/React 检索网格布局与相似推荐面板的连接。
...
* **团队分工**：
  ...
  * **袁晨宇**：完成前端界面基础布局和接口请求对接。

#### **第二阶段：大规模数据索引与黄金测试集标注（第 3 周）**
...
#### **第四阶段：LLM Agent 与 RAG 管道集成（第 5 周）**
...
* **交付产物 (Explicit Outputs)**：
  ...
  3. **Docker 编排配置**：编写 `Dockerfile` 与 `docker-compose.yml` 配置文件，打包前端与后端。
* **团队分工**：
  * **袁晨宇**：编写 MAP/nDCG 评估脚本，绘制 t-SNE / UMAP 聚类图。
  * **张艺升**：编写 Dockerfile 进行容器化打包，进行跨平台依赖管理。
